	...
	*/
	...
	void UserInput() {
		if (Input.GetMouseButton(0)) {
            ...
            RaycastHit hit2;
            //Color color = Physics.Raycast(ray, out hit, 100f) ? Color.green : Color.red;
            //Debug.DrawLine(ray.origin, ray.origin + (ray.direction * 100f), color);
            if (Physics.Raycast(ray, out hit2, 100))
            {
                // determine indices where the user clicked
                int x = (int)(hit2.point.x);
				int y = (int)(hit2.point.z);
                if (x < 1 || x >= lx || y < 1 || y >= ly) return;
                // add particles
                Particle2D newPart = Instantiate(part) as Particle2D;
				newPart.transform.position = hit2.point + Vector3.up;
